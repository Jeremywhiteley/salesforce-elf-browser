class EventLogFilesController < ApplicationController
  ...
  def index
    ...
    begin
      ...
      @log_files = @client.query("SELECT convertTimezone(logintime), userid, Application, SourceIp FROM LoginHistory where logintime >= #{date_to_time(@start_date)} AND logintime <= #{date_to_time(@end_date)} AND (hour_in_day(convertTimezone(logintime)) > #{@end_time} or hour_in_day(convertTimezone(logintime)) < #{@start_time}) and userid not in ('00561000001p5NAAAY', '00561000001atvkAAA', '00561000001ZZlDAAW', '00561000001ZZlQAAW', '00561000001ZZksAAG', '005610000013vc5AAA', '005610000013K1yAAE','00561000001p5N5AAI','00561000001a1haAAA','00550000003Y5UIAA0') AND SourceIP <> '207.211.26.117' ORDER BY logintime")
